...
test("rejects a transaction token tampered after signing", () => {
  ...
  const forgedPayload = Buffer.from(
    JSON.stringify({ state: "attacker-state", nonce: "nonce-1", code_verifier: "verifier-1", return_path: "/pages/learn", iat: 0, exp: 9_999_999_999 })
  )
    .toString("base64")
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
...
test("rejects an expired transaction (past the ~10 minute lifetime)", () => {
  ...
  const token = createOAuthTransaction({ state: "s", nonce: "n", codeVerifier: "v", returnPath: "/pages/learn", secret: SECRET, now: fixedNow });
...
test("rejects a transaction verified with the wrong secret", () => {
  const token = createOAuthTransaction({ state: "s", nonce: "n", codeVerifier: "v", returnPath: "/pages/learn", secret: SECRET });
...
test("createOAuthTransaction refuses to create an incomplete transaction", () => {
  assert.throws(() => createOAuthTransaction({ state: "", nonce: "n", codeVerifier: "v", returnPath: "/pages/learn", secret: SECRET }));
  assert.throws(() => createOAuthTransaction({ state: "s", nonce: "", codeVerifier: "v", returnPath: "/pages/learn", secret: SECRET }));
  assert.throws(() => createOAuthTransaction({ state: "s", nonce: "n", codeVerifier: "", returnPath: "/pages/learn", secret: SECRET }));
  assert.throws(() => createOAuthTransaction({ state: "s", nonce: "n", codeVerifier: "v", returnPath: "", secret: SECRET }));
});

test("rejects a validly-signed transaction payload missing return_path", () => {
  const token = signPayload({ state: "s", nonce: "n", code_verifier: "v" }, SECRET, 600);
  const result = verifyOAuthTransaction(token, { secret: SECRET });
  assert.equal(result.valid, false);
  assert.equal(result.reason, "incomplete_transaction");
});
